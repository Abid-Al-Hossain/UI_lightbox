...
import { DEFAULT_LIGHTBOX_STATE } from "./_data/LightboxPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("lightbox-component");
  ...
  const update = <K extends keyof LightboxState>(key: K, value: LightboxState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_LIGHTBOX_STATE, ...(preset.state as Partial<LightboxState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
