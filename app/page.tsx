...
import { DEFAULT_LIGHTBOX_STATE, LIGHTBOX_PRESETS } from "./_data/LightboxPresets";
...
import { SECTIONS, type SectionId, type LightboxState, type StudioPreset } from "./types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<LightboxState>(DEFAULT_LIGHTBOX_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_LIGHTBOX_STATE, LIGHTBOX_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("lightbox-component");
  ...
  const update = <K extends keyof LightboxState>(key: K, value: LightboxState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_LIGHTBOX_STATE, ...(preset.state as Partial<LightboxState>) }); setPreviewResetKey((value) => value + 1); };
  const exportPayload = useMemo(() => buildExportPayload(state, downloadName), [downloadName, state]);
  const preview = useMemo(
    () => (
      <LivePreview
        key={`${previewResetKey}:${state.activeIndex}:${state.previewState}:${state.mediaCount}:${state.label}`}
        state={state}
      />
    ),
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Lightbox Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
